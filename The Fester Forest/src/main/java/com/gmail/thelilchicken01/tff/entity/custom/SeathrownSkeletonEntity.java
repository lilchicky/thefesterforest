...
public class SeathrownSkeletonEntity extends ModWaterMonster implements IAnimatable {
	...
	private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
		...
		if(event.isMoving()) {
			event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.seathrown_skeleton.walk", EDefaultLoopTypes.LOOP));
			return PlayState.CONTINUE;
		}
		
		event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.seathrown_skeleton.idle", EDefaultLoopTypes.LOOP));
		return PlayState.CONTINUE;
